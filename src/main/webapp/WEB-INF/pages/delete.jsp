	...
	<form id="dataForm" name="dataForm">
		<div class="content">
			...
			<div class="clearfix" style="margin-top: 30px;">
				<input type="button" onclick="deleteFinger();" class="sub" value="<spring:message code="delete" />" /><span><spring:message code="step_delete_finger" /></span>
			</div>
		</div>
	</form>
  		<script type="text/javascript">
			function deleteFinger() {
				var name = $("#name").val();
				if(name.replace(/\ /g,'')==""){
					$("#name_ck").addClass("error").html("<spring:message code="require_delete_id" />").addClass("");	
					return;
				}else{
					$("#name_ck").removeClass("error").html("<spring:message code="step_input_id" />");	
				}
		      	updateInfo("delete","dataForm","result");
			}
  		</script>
  </body>
</html>
